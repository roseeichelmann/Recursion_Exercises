...
def groupSum5(start, nums, target):
    ...
    elif (nums[start - 1] % 5 == 0 and nums[start] == 1):
...
def groupSumClump(start, nums, target):
    if (start >= len(nums)):
        ...
        index = 0
        sum = 0
        while(nums[start] == nums[start + index]):
            sum += nums[start + index]
            index += 1
            if(start + index == len(nums)):
                break
        return groupSumClump(start + index, nums, target - sum) or groupSumClump(start + index, nums, target)
...
def splitArray(nums):
    ...
    sum1 = 0
    sum2 = 0
    goal_length = len(nums)
    return splitArrayHelper(group1, group2, sum1, sum2, goal_length, nums)


def splitArrayHelper(group1, group2, sum1, sum2, goal_length, nums):
    if (len(nums) == 0 and len(group1) + len(group2) == goal_length):
        return (sum1 == sum2)
    else:
        group1_append = group1[:]
        group1_append.append(nums[0])
        group2_append = group2[:]
        group2_append.append(nums[0])
        pop_val = nums.pop(0)
        result = (splitArrayHelper(group1_append, group2, sum1 + pop_val, sum2, goal_length, nums)) or (splitArrayHelper(group1, group2_append, sum1, sum2 + pop_val, goal_length, nums))
        nums.append(pop_val)
        return result
...
def splitOdd10(nums):
    ...
    sum2 = 0
    goal_length = len(nums)
    return splitOdd10Helper(group1, group2, sum1, sum2, goal_length, nums)

  
def splitOdd10Helper(group1, group2, sum1, sum2, goal_length, nums):
    if (len(nums) == 0 and len(group1) + len(group2) == goal_length):
        ...
        group2_append.append(nums[0])
        pop_val = nums.pop(0)
        result = (splitOdd10Helper(group1_append, group2, sum1 + pop_val, sum2, goal_length, nums)) or (splitOdd10Helper(group1, group2_append, sum1, sum2 + pop_val, goal_length, nums))
        nums.append(pop_val)
        return result
...
def split53(nums):
    ...
    sum2 = 0
    goal_length = len(nums)
    return split53Helper(group1, group2, sum1, sum2, goal_length, nums)

def split53Helper(group1, group2, sum1, sum2, goal_length, nums):
    if (len(nums) == 0 and len(group1) + len(group2) == goal_length):
        return (sum1 == sum2)
    elif (nums[0] % 5 == 0):
        group1.append(nums[0])
        # group1_append = group1[:]
        # group1_append.append(nums[0])
        pop_val = nums.pop(0)
        result = split53Helper(group1, group2, sum1 + pop_val, sum2, goal_length, nums)
        nums.append(pop_val)
        return result
    elif (nums[0] % 3 == 0):
        group2.append(nums[0])
        pop_val = nums.pop(0)
        result = split53Helper(group1, group2, sum1, sum2 + pop_val, goal_length, nums)
        nums.append(pop_val)
        return result
    else:
        group1_append = group1[:]
        ...
        group2_append.append(nums[0])
        pop_val = nums.pop(0)
        result = (split53Helper(group1_append, group2, sum1 + pop_val, sum2, goal_length, nums)) or (split53Helper(group1, group2_append, sum1, sum2 + pop_val, goal_length, nums))
        nums.append(pop_val)
        return result
